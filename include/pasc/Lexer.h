...
struct Location {
  ...
  static constexpr int ncontext = 5;
  static constexpr int linew = 5;
  void print() {
    std::cerr << *file << ":" << line << ":" << col << "\n";
    std::ifstream f(*file);
    std::string sline;
    for (int l=0; std::getline(f, sline) && l<line; l++) {
      if (line - l <= ncontext)
        std::cerr << std::setw(linew) << (l - line) << " | " << sline << "\n";
      // else std::cerr << line << " " << l << " " << sline << "\n";
    }
    for (int i=0; i < linew + 2; i++)
      std::cerr << " ";
    for (int i=0; i < col; i++)
      std::cerr << " ";
    std::cerr << "^\n";
    for (int l=0; std::getline(f, sline) && l<ncontext; l++) {
      std::cerr << std::setw(linew) << l << " | " << sline << "\n";
    }
  }
};

inline llvm::raw_ostream& operator<<(llvm::raw_ostream& os, Location const& l) {
  os << *(l.file) << ":" << l.line << ":" << l.col;
  return os;
}
...
enum Token : int {
  ...
  // pascal comments are usually just single curly brace so I'm doing double
  // so the existing toy language parsing doesn't get confused
  tok_comment_open = '{',
  tok_comment_close = '}',
...
inline llvm::raw_ostream& operator<<(llvm::raw_ostream& os, const Token& t)
{
  switch (t) {
    case tok_block_open: os << "begin"; break;
    case tok_block_close: os << "end"; break;
    case tok_return: os << "return"; break;
    case tok_var: os << "var"; break;
    case tok_def: os << "def"; break;
    case tok_for: os << "for"; break;
    default: os << (char)t; break;
  }
  return os;
}
...
public:
  ...
  void consume(Token tok) {
    if (tok != curTok) {
      lastLocation.print();
      llvm::errs() << "Expected " << tok << " got " << curTok << "\n";
      assert(false && "consume Token mismatch expectation");
    }
    getNextToken();
  }
  ...
  llvm::StringRef getCurLineBuffer() { return curLineBuffer; }
...
private:
  ...
  Token getTok() {
    ...
    if (isalpha(lastChar)) {
      ...
      if (identifierStr == "return")
        return tok_return;
      if (identifierStr == "def" || identifierStr == "function")
        return tok_def;
  ...
  void eatComments() {
    if (lastChar == '{') {
      // Comment until end of comment block
      while (lastChar != '}')
        lastChar = Token(getNextChar());
      lastChar = Token(getNextChar());
      // consume(tok_comment_close);
    }
    eatSpace();
  }
