...
public:
  AssignExprAST(Location loc, llvm::StringRef name, std::unique_ptr<ExprAST> rhs)
      : ExprAST(Expr_Assign, std::move(loc)), name(name), rhs(std::move(rhs)) {}
...
/// For loop with init, test, incr, and body
...
public:
  ForLoopAST(Location loc, std::unique_ptr<ExprAST> init,
             std::unique_ptr<ExprAST> test, std::unique_ptr<ExprAST> incr,
             std::unique_ptr<ExprASTList> body)
      : ExprAST(Expr_ForLoop, std::move(loc)),
        init(std::move(init)), test(std::move(test)),
        incr(std::move(incr)), body(std::move(body)) {}

  ExprAST *getInit() { return init.get(); }
  ExprAST *getTest() { return test.get(); }
  ExprAST *getIncr() { return incr.get(); }
