...
private:
  ...
  std::unique_ptr<AssignExprAST> parseAssignment() {
    if (lexer.getCurToken() != tok_identifier)
      return parseError<AssignExprAST>("<identifier> = <rhs>;", "to assign to a variable");
    auto loc = lexer.getLastLocation();

    std::string id(lexer.getId());
    lexer.getNextToken(); // eat id
    lexer.consume(tok_eq);

    auto rhs = parseExpression();
    return std::make_unique<AssignExprAST>(std::move(loc), std::move(id),
                                           std::move(rhs));
  }
  ...
  std::unique_ptr<VarDeclExprAST> parseDeclaration() {
    ...
    return std::make_unique<VarDeclExprAST>(std::move(loc), std::move(id),
                                            std::move(*type), std::unique_ptr<ExprAST>());
  }
  ...
  std::unique_ptr<ExprASTList> parseBlock() {
    if (lexer.getCurToken() != tok_block_open)
      return parseError<ExprASTList>("begin", "to begin block");
    lexer.consume(tok_block_open);
    ...
    while (lexer.getCurToken() != tok_block_close && lexer.getCurToken() != tok_eof) {
      if (lexer.getCurToken() == tok_var) {
        ...
        if (!loop)
          return nullptr;
        exprList->push_back(std::move(loop));
      } else {
        // General expression
        auto expr = parseExpression();
        if (!expr)
    ...
    if (lexer.getCurToken() != tok_block_close)
      return parseError<ExprASTList>("end", "to close block");

    lexer.consume(Token(tok_block_close));
    lexer.consume(Token(';'));
    return exprList;
  }

  // forloop ::= for <expr>; <expr>; <expr>; begin; <expr_list>; end;
  std::unique_ptr<ForLoopAST> parseForLoop() {
    ...
    if (!body)
      return nullptr;
    lexer.consume(tok_semicolon);
    return std::make_unique<ForLoopAST>(std::move(loc), std::move(init),
                                        std::move(test), std::move(incr),
                                        std::move(body));
  }
